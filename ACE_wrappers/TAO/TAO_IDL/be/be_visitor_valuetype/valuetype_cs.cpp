 ...
 *
 *  $Id: valuetype_cs.cpp 97145 2013-05-17 13:42:03Z sma $
 *
  ...
  if (!node->is_abstract () && !is_an_amh_exception_holder)
    {
      ...
      if  (node->custom ())
        {
          ...
          *os << "::CORBA::DataOutputStream_var st;" << be_nl
              << "ACE_NEW_RETURN (st, ::CORBA::TAO_DataOutputStream (strm), false);" << be_nl
              << "const_cast < " << scope->name () << "::"
              << node->local_name () << "* > (this)->marshal (st);" << be_nl
              << "return true;" << be_uidt_nl;
        }
      else
        {
          *os << "TAO_ChunkInfo ci (this->is_truncatable_ || this->chunking_);"
              << be_nl
              << "return ";
      ...
      *os << "::CORBA::Boolean" << be_nl
      ...
      if (node->custom ())
        {
          *os << "::CORBA::DataInputStream_var st;" << be_nl
              << "ACE_NEW_RETURN (st, ::CORBA::TAO_DataInputStream (strm), false);" << be_nl
              << "this->unmarshal (st);" << be_nl
              << "return true;" << be_uidt_nl;
        }
      else
        {
          *os << "TAO_ChunkInfo ci (this->is_truncatable_ || this->chunking_, 1);"
              << be_nl
              << "return ";
